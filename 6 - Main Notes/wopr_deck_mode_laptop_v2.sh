...
check_nvidia_kernel_params() {
  ...
  local nvidia_gpu
  nvidia_gpu=$(lspci | grep -i nvidia | grep -iE 'vga|3d|display' | head -1)
  info "NVIDIA GPU detected: ${nvidia_gpu:-unknown}"
...
configure_limine_nvidia() {
  ...
  # Match cmdline: anywhere in the line (handles indented entries in boot menus)
  # The pattern matches lines containing 'cmdline:' followed by content, and appends the param
  if sudo sed -i '/cmdline:.*[^[:space:]]/ s/$/ nvidia-drm.modeset=1/' "$config_file"; then
    ...
    if grep -q "nvidia-drm.modeset=1" "$config_file"; then
      ...
      err "Failed to add parameter to Limine config"
      echo ""
      echo "  The sed pattern did not match any 'cmdline:' lines in $config_file"
      echo "  This may indicate an unusual Limine config format."
      echo ""
      echo "  Your Limine config contains these lines with 'cmdline':"
      grep -n "cmdline" "$config_file" 2>/dev/null | head -5 || echo "    (no cmdline lines found)"
      echo ""
      show_manual_nvidia_instructions
    fi
  else
    err "Failed to modify Limine config (permission denied?)"
    show_manual_nvidia_instructions
  fi
}
...
# Returns: 0 if lid is closed, 1 if open or not a laptop
is_lid_closed() {
  # Method 1: Check ACPI lid state (most reliable)
  local lid_state_file=""
  for f in /proc/acpi/button/lid/*/state /proc/acpi/button/lid/LID0/state /proc/acpi/button/lid/LID/state; do
    [[ -f "$f" ]] && lid_state_file="$f" && break
  done
  if [[ -n "$lid_state_file" ]]; then
    if grep -qi "closed" "$lid_state_file" 2>/dev/null; then
      return 0  # Lid is closed
    fi
    return 1  # Lid is open
  fi

  # Method 2: Check if eDP/LVDS connector is disabled via sysfs
  for card_path in /sys/class/drm/card[0-9]*; do
    local card_name
    card_name=$(basename "$card_path")
    [[ "$card_name" == render* ]] && continue
    for connector in "$card_path"/"$card_name"-{eDP,LVDS}*/enabled; do
      if [[ -f "$connector" ]] && grep -qi "disabled" "$connector" 2>/dev/null; then
        return 0  # Internal display disabled = lid likely closed
      fi
    done
  done

  return 1  # Assume lid is open if we can't determine
}

# Check if a connector is an internal display (eDP or LVDS)
is_internal_connector() {
  local connector="$1"
  [[ "$connector" == *-eDP-* ]] || [[ "$connector" == *-LVDS-* ]]
}
...
# Enhanced: Handles multiple displays, lid state, and prioritizes correctly
get_display_gpu_type() {
  local lid_closed=false
  is_lid_closed && lid_closed=true

  # Collect ALL connected displays with their GPU type and connector type
  local -a internal_displays=()   # eDP/LVDS displays (laptop screen)
  local -a external_nvidia=()     # External displays on NVIDIA
  local -a external_igpu=()       # External displays on Intel/AMD iGPU
  local -a external_amd_dgpu=()   # External displays on AMD dGPU
  ...
  for card_path in /sys/class/drm/card[0-9]*; do
    ...
    for connector in "$card_path"/"$card_name"-*/status; do
      ...
      grep -q "^connected$" "$connector" 2>/dev/null || continue

      local connector_name
      connector_name=$(basename "$(dirname "$connector")")

      # Categorize by connector type and GPU
      if is_internal_connector "$connector_name"; then
        # Internal display (eDP/LVDS) - always on iGPU
        case "$driver" in
          i915|xe) internal_displays+=("intel:$connector_name") ;;
          amdgpu)
            if is_amd_igpu_card "$card_path"; then
              internal_displays+=("amd-igpu:$connector_name")
            fi
            ;;
        esac
      else
        # External display (HDMI, DP, VGA, DVI)
        case "$driver" in
          nvidia) external_nvidia+=("$connector_name") ;;
          i915|xe) external_igpu+=("intel:$connector_name") ;;
          amdgpu)
            if is_amd_igpu_card "$card_path"; then
              external_igpu+=("amd-igpu:$connector_name")
            else
              external_amd_dgpu+=("$connector_name")
            fi
            ;;
        esac
      fi
  ...
  # Decision logic based on what's connected and lid state
  local num_external_nvidia=${#external_nvidia[@]}
  local num_external_igpu=${#external_igpu[@]}
  local num_external_amd_dgpu=${#external_amd_dgpu[@]}
  local num_internal=${#internal_displays[@]}

  # Debug logging (visible in journal)
  if [[ "${DEBUG_GPU_DETECT:-}" == "1" ]]; then
    logger -t gpu-detect "lid_closed=$lid_closed internal=$num_internal ext_nvidia=$num_external_nvidia ext_igpu=$num_external_igpu ext_amd_dgpu=$num_external_amd_dgpu"
  fi

  # Case 1: Lid closed - only consider external displays
  if $lid_closed; then
    if ((num_external_nvidia > 0)); then
      echo "nvidia"
      return 0
    elif ((num_external_amd_dgpu > 0)); then
      echo "amd-dgpu"
      return 0
    elif ((num_external_igpu > 0)); then
      # External on iGPU with lid closed - still hybrid for rendering
      local igpu_type="${external_igpu[0]%%:*}"
      echo "$igpu_type"
      return 0
    fi
  fi

  # Case 2: External display on NVIDIA (lid open or no internal)
  # This indicates user wants to use NVIDIA for display (desktop-style)
  if ((num_external_nvidia > 0)) && ((num_internal == 0)); then
    echo "nvidia"
    return 0
  fi

  # Case 3: External on AMD dGPU only
  if ((num_external_amd_dgpu > 0)) && ((num_internal == 0)); then
    echo "amd-dgpu"
    return 0
  fi

  # Case 4: Internal display connected (laptop screen in use)
  # This is hybrid mode - iGPU displays, dGPU renders
  if ((num_internal > 0)); then
    local igpu_type="${internal_displays[0]%%:*}"
    echo "$igpu_type"
    return 0
  fi

  # Case 5: Only external on iGPU (rare but possible)
  if ((num_external_igpu > 0)); then
    local igpu_type="${external_igpu[0]%%:*}"
    echo "$igpu_type"
    return 0
  fi

  # Note: Case "external NVIDIA + internal" is handled by Case 4 (internal present = hybrid)
...
setup_session_switching() {
  ...
  if command -v hyprctl >/dev/null 2>&1; then
    ...
    if [[ -n "$monitor_json" ]] && command -v jq >/dev/null 2>&1; then
      local monitor_count
      monitor_count=$(echo "$monitor_json" | jq 'length' 2>/dev/null) || monitor_count=0
      [[ "$monitor_count" =~ ^[0-9]+$ ]] || monitor_count=0

      if ((monitor_count > 1)); then
        # Multiple monitors detected - let user choose
        echo ""
        echo "=== MONITOR SELECTION ==="
        echo "Multiple monitors detected. Select which monitor to use for Gaming Mode:"
        echo ""

        local -a monitor_names=()
        local -a monitor_info=()
        local i=1

        while IFS= read -r mon; do
          local name width height refresh
          name=$(echo "$mon" | jq -r '.name')
          width=$(echo "$mon" | jq -r '.width')
          height=$(echo "$mon" | jq -r '.height')
          refresh=$(echo "$mon" | jq -r '.refreshRate | floor')
          monitor_names+=("$name")
          monitor_info+=("${width}x${height}@${refresh}Hz")
          echo "  $i) $name - ${width}x${height} @ ${refresh}Hz"
          ((i++))
        done < <(echo "$monitor_json" | jq -c '.[]')

        echo ""
        local choice
        while true; do
          read -p "Select monitor [1-$monitor_count] (default: 1): " choice
          choice=${choice:-1}
          if [[ "$choice" =~ ^[0-9]+$ ]] && ((choice >= 1 && choice <= monitor_count)); then
            break
          fi
          echo "Invalid choice. Please enter a number between 1 and $monitor_count."
        done

        local selected_index=$((choice - 1))
        local selected_monitor
        selected_monitor=$(echo "$monitor_json" | jq ".[$selected_index]")

        monitor_output=$(echo "$selected_monitor" | jq -r '.name')
        monitor_width=$(echo "$selected_monitor" | jq -r '.width')
        monitor_height=$(echo "$selected_monitor" | jq -r '.height')
        monitor_refresh=$(echo "$selected_monitor" | jq -r '.refreshRate | floor')

        # Get refresh rates only from the SELECTED monitor's supported modes
        # For now, use the current refresh rate as the primary
        all_refresh_rates="$monitor_refresh"

        info "Selected monitor: $monitor_output (${monitor_width}x${monitor_height}@${monitor_refresh}Hz)"
      elif ((monitor_count == 1)); then
        # Single monitor - auto-select
        local single_monitor
        single_monitor=$(echo "$monitor_json" | jq '.[0]')
        monitor_output=$(echo "$single_monitor" | jq -r '.name // ""')
        monitor_width=$(echo "$single_monitor" | jq -r '.width // 1920')
        monitor_height=$(echo "$single_monitor" | jq -r '.height // 1080')
        monitor_refresh=$(echo "$single_monitor" | jq -r '.refreshRate // 60 | floor')
        all_refresh_rates="$monitor_refresh"
      fi
      # If monitor_count == 0 (empty array), keep defaults
    fi
  fi

  # Fallback defaults
  [[ -z "$all_refresh_rates" ]] && all_refresh_rates="$monitor_refresh"

  info "Gaming Mode display: ${monitor_width}x${monitor_height}@${monitor_refresh}Hz${monitor_output:+ on $monitor_output}"

  # Detect keyboard layout from system
  local keyboard_layout=""
  local keyboard_variant=""

  # Method 1: Check Hyprland config for keyboard layout
  if [[ -f "${user_home}/.config/hypr/hyprland.conf" ]]; then
    # Pattern matches: letters, numbers, underscores, hyphens, commas (for multi-layout like "us,de")
    keyboard_layout=$(grep -oP 'kb_layout\s*=\s*\K[a-zA-Z0-9_,\-]+' "${user_home}/.config/hypr/hyprland.conf" 2>/dev/null | head -1)
    keyboard_variant=$(grep -oP 'kb_variant\s*=\s*\K[a-zA-Z0-9_,\-]+' "${user_home}/.config/hypr/hyprland.conf" 2>/dev/null | head -1)
  fi

  # Method 2: Check localectl if Hyprland config doesn't have it
  if [[ -z "$keyboard_layout" ]] && command -v localectl >/dev/null 2>&1; then
    local localectl_out
    localectl_out=$(localectl status 2>/dev/null)
    keyboard_layout=$(echo "$localectl_out" | grep -oP 'X11 Layout:\s*\K\S+' | head -1)
    keyboard_variant=$(echo "$localectl_out" | grep -oP 'X11 Variant:\s*\K\S+' | head -1)
  fi

  # Method 3: Check /etc/vconsole.conf
  if [[ -z "$keyboard_layout" ]] && [[ -f /etc/vconsole.conf ]]; then
    keyboard_layout=$(grep -oP '^KEYMAP=\K\S+' /etc/vconsole.conf 2>/dev/null | head -1)
  fi
  ...
  if [[ -n "$keyboard_variant" ]]; then
    info "Detected keyboard layout: ${keyboard_layout} (variant: ${keyboard_variant})"
  else
...
UDISKS_POLKIT
  ...
  # Build keyboard layout line for config
  local keyboard_config="XKB_DEFAULT_LAYOUT=${keyboard_layout}"
  [[ -n "$keyboard_variant" ]] && keyboard_config="${keyboard_config}
...
CUSTOM_REFRESH_RATES=${all_refresh_rates}
${output_connector}

# Keyboard layout (auto-detected from system)
${keyboard_config}
...
# Enhanced: Proper lid detection, multi-display handling for AMD iGPU + NVIDIA dGPU laptops
...
# Check if laptop lid is closed
is_lid_closed() {
    # Method 1: ACPI lid state
    for f in /proc/acpi/button/lid/*/state; do
        [[ -f "$f" ]] && grep -qi "closed" "$f" 2>/dev/null && return 0
    done
    # Method 2: Check if eDP is disabled
    for card_path in /sys/class/drm/card[0-9]*; do
        local card_name=$(basename "$card_path")
        [[ "$card_name" == render* ]] && continue
        for conn in "$card_path"/"$card_name"-eDP*/enabled "$card_path"/"$card_name"-LVDS*/enabled; do
            [[ -f "$conn" ]] && grep -qi "disabled" "$conn" 2>/dev/null && return 0
        done
    done
    return 1
}

# Check if connector is internal (eDP/LVDS)
is_internal_connector() {
    [[ "$1" == *-eDP-* ]] || [[ "$1" == *-LVDS-* ]]
}
...
is_amd_igpu_card() {
    ...
    # Check for AMD iGPU codenames (complete list)
    if echo "$device_info" | grep -iqE 'renoir|cezanne|barcelo|rembrandt|phoenix|raphael|lucienne|picasso|raven|vega.*mobile|vega.*integrated|radeon.*graphics|yellow.*carp|green.*sardine|cyan.*skillfish|vangogh|van gogh|mendocino|hawk.*point|strix.*point|strix.*halo|krackan|sarlak'; then
        return 0
    fi
...
# Get display GPU type with proper multi-display and lid handling
get_display_gpu_type() {
    local lid_closed=false
    is_lid_closed && lid_closed=true

    # Collect all connected displays
    local -a internal_displays=()
    local -a external_nvidia=()
    local -a external_igpu=()
    local -a external_amd_dgpu=()
    ...
    for card_path in /sys/class/drm/card[0-9]*; do
        ...
        for connector in "$card_path"/"$card_name"-*/status; do
            ...
            if is_internal_connector "$connector_name"; then
                case "$driver" in
                    i915|xe) internal_displays+=("intel") ;;
                    amdgpu) is_amd_igpu_card "$card_path" && internal_displays+=("amd-igpu") ;;
                esac
            else
                case "$driver" in
                    nvidia) external_nvidia+=("$connector_name") ;;
                    i915|xe) external_igpu+=("intel") ;;
                    amdgpu)
                        if is_amd_igpu_card "$card_path"; then
                            external_igpu+=("amd-igpu")
                        else
                            external_amd_dgpu+=("$connector_name")
                        fi
                        ;;
    ...
    # Decision logic
    if $lid_closed; then
        ((${#external_nvidia[@]} > 0)) && { echo "nvidia"; return 0; }
        ((${#external_amd_dgpu[@]} > 0)) && { echo "amd-dgpu"; return 0; }
        ((${#external_igpu[@]} > 0)) && { echo "${external_igpu[0]}"; return 0; }
    fi

    # External on NVIDIA only (no internal) = desktop mode
    ((${#external_nvidia[@]} > 0)) && ((${#internal_displays[@]} == 0)) && { echo "nvidia"; return 0; }

    # External on AMD dGPU only = desktop mode
    ((${#external_amd_dgpu[@]} > 0)) && ((${#internal_displays[@]} == 0)) && { echo "amd-dgpu"; return 0; }

    # Internal display = hybrid mode (iGPU displays)
    ((${#internal_displays[@]} > 0)) && { echo "${internal_displays[0]}"; return 0; }

    # External on iGPU
    ((${#external_igpu[@]} > 0)) && { echo "${external_igpu[0]}"; return 0; }

    # Dual display (internal + external NVIDIA) = hybrid
    ((${#external_nvidia[@]} > 0)) && ((${#internal_displays[@]} > 0)) && { echo "${internal_displays[0]}"; return 0; }
...
detect_gpu_mode() {
    ...
    # Complete AMD iGPU pattern matching
    echo "$gpu_info" | grep -iqE 'renoir|cezanne|barcelo|rembrandt|phoenix|raphael|lucienne|picasso|raven|radeon.*graphics|vangogh|van gogh|mendocino|hawk.*point|strix.*point|strix.*halo|yellow.*carp|green.*sardine|cyan.*skillfish|krackan|sarlak' && has_amd_igpu=true

    if $has_nvidia && { $has_intel || $has_amd_igpu; }; then
        local display_type=$(get_display_gpu_type)
        case "$display_type" in
            nvidia) echo "nvidia" ;;
            intel|amd-igpu) echo "hybrid" ;;
            amd-dgpu) echo "nvidia" ;;
            *)
                if is_laptop; then
                    echo "hybrid"
                else
...
get_display_gpu_type() {
    ...
    log "Display detection: lid_closed=$lid_closed internal=${#internal_displays[@]} ext_nvidia=${#external_nvidia[@]} ext_igpu=${#external_igpu[@]}"
...
# Runtime display detection - override static config if different monitor is connected
detect_runtime_display() {
    local config_file="$HOME/.config/environment.d/gamescope-session-plus.conf"
    [[ ! -f "$config_file" ]] && return

    # Try to detect current display using DRM sysfs (works without Hyprland running)
    local best_width=0 best_height=0 best_output=""
    ...
    for card_path in /sys/class/drm/card[0-9]*; do
        ...
        for mode_path in "$card_path"/"$card_name"-*/modes; do
            [[ -f "$mode_path" ]] || continue
            local connector_dir=$(dirname "$mode_path")
            local status_file="$connector_dir/status"
            [[ -f "$status_file" ]] && grep -q "^connected$" "$status_file" || continue

            local connector_name=$(basename "$connector_dir")
            local is_internal=false
            [[ "$connector_name" == *eDP* ]] || [[ "$connector_name" == *LVDS* ]] && is_internal=true

            # Skip internal display if lid is closed
            if $is_internal && is_lid_closed; then
                continue
            fi

            # Parse modes file - first line is preferred mode (WIDTHxHEIGHT format)
            # Note: DRM modes file does NOT contain refresh rate, only resolution
            local mode=$(head -1 "$mode_path" 2>/dev/null)
            if [[ "$mode" =~ ^([0-9]+)x([0-9]+) ]]; then
                ...
                if ! $is_internal; then
                    # External monitor found - use it
                    if ! $found_external || ((w * h > best_width * best_height)); then
                        best_width=$w
                        best_height=$h
                        best_output=$connector_name
                        found_external=true
                    fi
                elif ! $found_external; then
                    # No external yet, use internal if it's better
                    if ((w * h > best_width * best_height)); then
                        best_width=$w
                        best_height=$h
                        best_output=$connector_name
                    fi
    ...
    # If we detected a display, export resolution override
    # Note: We do NOT override CUSTOM_REFRESH_RATES - the static config has that from install time
    # Gamescope will use the monitor's EDID to determine available refresh rates
    if ((best_width > 0)); then
        log "Runtime display: ${best_width}x${best_height} on $best_output"

        # Only export if different from what might be in config
        export SCREEN_WIDTH="$best_width"
        export SCREEN_HEIGHT="$best_height"
        [[ -n "$best_output" ]] && export OUTPUT_CONNECTOR="$best_output"
        # Don't override CUSTOM_REFRESH_RATES - let gamescope detect from EDID
    fi
}
